...
board = cv.imread(r"King Domino dataset\Cropped and perspective corrected boards\19.jpg", 1)
board_gray = cv.cvtColor(board, cv.COLOR_BGR2GRAY)
template = cv.imread(r'Crown cropped.png',0)
template = cv.resize(template, [33,26], interpolation = cv.INTER_AREA)
